...
func RegisterCommands(app *cobra.Command, c *client.Client) {
	...
	command = &cobra.Command{
		Use:   "multiply",
		Short: `add returns the product of the left and right parameters in the response body`,
	}
	tmp2 := new(MultiplyOperandsCommand)
	sub = &cobra.Command{
		Use:   `operands ["/multiply/LEFT/RIGHT"]`,
		Short: ``,
	...
	command = &cobra.Command{
		Use:   "subtract",
		Short: `subtract returns the difference of the left and right parameters in the response body`,
	}
	tmp3 := new(SubtractOperandsCommand)
	sub = &cobra.Command{
...
func (cmd *MultiplyOperandsCommand) Run(c *client.Client, args []string) error {
	...
	if len(args) > 0 {
		...
		path = fmt.Sprintf("/multiply/%v/%v", cmd.Left, cmd.Right)
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.MultiplyOperands(ctx, path)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *MultiplyOperandsCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var left int
	cc.Flags().IntVar(&cmd.Left, "left", left, `Left operand`)
	var right int
	cc.Flags().IntVar(&cmd.Right, "right", right, `Right operand`)
